...
useEffect( () => {
  ...
  return (
    <>
      <main className='flex justify-evenly items-center h-screen scale-150 ' >
        <div className='flex justify-between items-center flex-col'>
          {
            svgnumbers.map((_,i)=>{
              return <p key={i} style={{color:i == index ? "red" : "black" , cursor:"pointer"}} onClick={()=>{setIndex(i)}}> {i + 1} </p>
            })
          }
        </div>
